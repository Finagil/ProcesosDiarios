...
Module LayoutBancomer
    Public Function EnviaLayoutNORMAL(ByVal cTipoReporte As String, Dias As Integer) As String
        Dim taJur As New ProduccionDSTableAdapters.JUR_NoDomiciliarAvisoTableAdapter
        Dim taDiasFest As New ProduccionDSTableAdapters.GEN_DiasFestivosTableAdapter
        Dim tDiaFest As New ProduccionDS.GEN_DiasFestivosDataTable
        ...
        taDiasFest.Fill(tDiaFest, Hoy)
        Dim nDiaSemana As Byte = Hoy.Date.DayOfWeek

        If tDiaFest.Rows.Count > 0 Then ' dias festivos
            Exit Function ' no se genera archivos en dias Festivos en proceso normal 
        End If
        ...
        If lProcesar = True Then
            ...
            If cTipoReporte = "B" Then
                ...
                Select Case nDiaSemana
                    Case 1 To 4             ' Lunes a Jueves se lleva dia siguiente
            ...
            ElseIf cTipoReporte = "O" Then
                ...
                Select Case nDiaSemana
                    ...
                    Case 5              ' viernes sabado y domingo
                        cFechaInicial = Hoy.AddDays(0).ToString("yyyyMMdd")
                        cFechaFinal = Hoy.AddDays(2).ToString("yyyyMMdd")
                End Select
            ...
            If dsAgil.Tables("Pagos").Rows.Count > 0 Then
                ...
                For Each drAnexo In dtDomiciliacion.Rows
                    ...
                    If cLeyenda.Length > 40 Then cLeyenda = cLeyenda.Substring(0, 40)
    ...
    Public Function EnviaLayoutFESTIVO(ByVal cTipoReporte As String, Dias As Integer) As String
        Dim taJur As New ProduccionDSTableAdapters.JUR_NoDomiciliarAvisoTableAdapter
        Dim taDiasFest As New ProduccionDSTableAdapters.GEN_DiasFestivosTableAdapter
        Dim tDiaFest As New ProduccionDS.GEN_DiasFestivosDataTable
        Dim cnAgil As New SqlConnection(My.Settings.ConnectionStringDOMI)
        ...
        'REVISA FECHA DE PROCESO
        Dim Hoy As Date = Today.AddDays(Dias)
        taDiasFest.Fill(tDiaFest, Hoy)
        If tDiaFest.Rows.Count > 0 Then ' SI LA FEHA DE PROCESO ES FESTIVO
            Exit Function ' NO DEBE HACER NADA
        End If

        'REVISA SI EL DIA HABIL SIGUIENTE ES FESTIVO
        Dim nDiaSemana As Byte = Hoy.Date.DayOfWeek
        Select Case nDiaSemana
            Case 5 'VIERNES
                Hoy = Today.AddDays(Dias + 3) ' LUNES ES FESTIVO ?
                taDiasFest.Fill(tDiaFest, Hoy)
                If tDiaFest.Rows.Count <= 0 Then '
                    Exit Function
                End If
            Case 1, 2, 3, 4       ' Lunes a Viernes
                Hoy = Today.AddDays(Dias + 1) ' DIA SIGUIENTE ES FESTIVO ?
                taDiasFest.Fill(tDiaFest, Hoy)
                If tDiaFest.Rows.Count <= 0 Then '
                    Exit Function
                End If
        ...
        If lProcesar = True Then
            ...
            If cTipoReporte = "B" Then
                ...
                Select Case nDiaSemana
                    Case 1 To 4             ' Lunes a Jueves se lleva dia siguiente
                        ...
                        cFechaInicialEXT = Hoy.AddDays(1).ToString("yyyyMMdd") ' hoy 
                        cFechaFinalEXT = Hoy.AddDays(1).ToString("yyyyMMdd")'mañana
                    Case 5                  ' Viernes 
                        cFechaInicial = Hoy.AddDays(1).ToString("yyyyMMdd") 'sabado
                        cFechaFinal = Hoy.AddDays(3).ToString("yyyyMMdd") 'domingo y lunes
            ...
            If dsAgil.Tables("Pagos").Rows.Count > 0 Then
                ...
                nCount = 1
                If cTipoReporte = "B" Then
                    writer = New StreamWriter(My.Settings.RUTA_TMP & "DOMI\Fest_Pagos_BANCOMER_" & Hoy.ToString("ddMMyyyy") & "_" & ContadorAux1 & ".txt")
                ElseIf cTipoReporte = "O" Then
                    writer = New StreamWriter(My.Settings.RUTA_TMP & "DOMI\Fest_Pagos_OTROS_BANCOS_" & Hoy.ToString("ddMMyyyy") & "_" & ContadorAux1 & ".txt")
                End If
            ...
            Else
                Try
                    ...
                    De = ("Domiciliacion@Finagil.com.mx")
                    If cTipoReporte = "B" Then
                        Asunto = "SIN DATOS - Layout BANCOMER FESTIVO"
                    ElseIf cTipoReporte = "O" Then
                        Asunto = "SIN DATOS - Layout OTROS BANCOS FESTIVO"
                    End If
                    MGlobal.EnviacORREO(Para, "", Asunto, De)
                    cMensaje = "Generación y envío exitosos"
                Catch ex As Exception
